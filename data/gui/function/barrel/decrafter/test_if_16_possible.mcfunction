...
scoreboard players set #required_count_display decraft 16
scoreboard players operation #required_count_display decraft *= SelectedItemCount decraft
# say test 16

execute at @s if data block ~ ~ ~ Items[{Slot:1b,id:"minecraft:barrier"}] run return run data modify storage minecraft:ui DecraftPossible.sixteen set value {Slot:22b, count:16,id:"minecraft:red_stained_glass_pane", components: {"minecraft:custom_name":{italic:0b, text:"Not enough item", bold:1b, color:"red"},"minecraft:custom_data":{ui_item:1,actions:{}}}}


execute if score #count_display_storage decraft < #required_count_display decraft run return run data modify storage minecraft:ui DecraftPossible.sixteen set value {Slot:22b, count:16,id:"minecraft:red_stained_glass_pane", components: {"minecraft:custom_name":{italic:0b, text:"Not enough item", bold:1b, color:"red"},"minecraft:custom_data":{ui_item:1,actions:{}}}}


data modify storage minecraft:ui DecraftPossible.sixteen set value {Slot:22b, count:16, id:"minecraft:lime_stained_glass_pane", components: {"minecraft:custom_name":{italic:0b, text:"Decraft 16", bold:1b, color:"green"},"minecraft:custom_data":{ui_item:1,actions:{on_click:{command:"function gui:barrel/decrafter/decraft_button {decraft_number:16}"}}}}}